...
@Component({
  selector: 'app-lineage-graph-dialog',
  templateUrl: './lineage-graph-dialog.component.html',
  styleUrls: ['./lineage-graph.component.css']
})
export class LineageGraphDialogComponent implements AfterViewInit {

  @ViewChild('pedigreeChartDialogContainer', { static: true }) private pedigreeChartDialogContainer: ElementRef;
  @ViewChild('horseHaploGroupChartDialogContainer', { static: true }) private horseHaploGroupChartDialogContainer: ElementRef;
  @ViewChild('populationHaploGroupChartDialogContainer', { static: true }) private populationHaploGroupChartDialogContainer: ElementRef;
  @Input() inputChartData: HorseHeirarchy;
  @Input() inputPopulationHaploGroups: HaploGroup[] = [];
  @Input() inputHorseHaploGroups: HaploGroup[] = [];
  @Input() inputHorseName: string;
  ...
  ngAfterViewInit() {
    ...
    this.horseHaploGroupTooltip = d3.select(this.horseHaploGroupChartDialogContainer.nativeElement)
      .append("div")
      .attr("class", "d3tooltip")
      .style("display", "none");

    this.populationHaploGroupTooltip = d3.select(this.populationHaploGroupChartDialogContainer.nativeElement)
      .append("div")
      .attr("class", "d3tooltip")
      .style("display", "none");
  }
  ...
  ngOnChanges() {
    if (!_.isEqual(this.chartData, this.inputChartData)) {
      if (!_.isEqual(this.horseName, this.inputHorseName)) {
        this.horseName = this.inputHorseName;
      }
      this.chartData = this.inputChartData;
      if(this.horseName != 'Hypothetical') {
        this.chartData.name = this.horseName
      }
      this.generateChart();
    }
    ...
    if (!_.isEqual(this.populationHaploGroups, this.inputPopulationHaploGroups)) {
      ...
      this.drawHaploGroupChart({
        svgElId: 'population-haplo-group-chart-dialog',
    ...
    if (!_.isEqual(this.horseHaploGroups, this.inputHorseHaploGroups)) {
      ...
      this.drawHaploGroupChart({
        svgElId: 'horse-haplo-group-chart-dialog',
  ...
  generateChart() {
    const element = this.pedigreeChartDialogContainer.nativeElement;
    const pedigreeChartNode = document.getElementById('pedigree-chart-dialog');

    const width = pedigreeChartNode.clientWidth;
    const height = pedigreeChartNode.clientHeight;

    const svg = d3.select('#pedigree-chart-dialog');
  ...
  mouseover(d) {
    this.tooltipDiv = d3.select(this.pedigreeChartDialogContainer.nativeElement).append("div")
      .attr("class", "d3tooltip")
      .style("opacity", 1)
